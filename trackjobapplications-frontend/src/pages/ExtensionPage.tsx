      ...
      <div className="max-w-3xl mx-auto space-y-6 pb-12">
        ...
        <div className="bg-white dark:bg-stone-900 rounded-lg border border-stone-200/60 dark:border-stone-800 shadow-sm p-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <a
              ...
              className="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-lg bg-stone-900 hover:bg-stone-800 dark:bg-white dark:hover:bg-stone-100 text-white dark:text-stone-900 font-medium text-sm transition-colors"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-lg border border-stone-200 dark:border-stone-700 text-stone-600 dark:text-stone-400 hover:bg-stone-50 dark:hover:bg-stone-800 font-medium text-sm transition-colors"
            >
              <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
        ...
        <div className="bg-white dark:bg-stone-900 rounded-lg border border-stone-200/60 dark:border-stone-800 shadow-sm p-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {['LinkedIn', 'Indeed', 'Glassdoor', 'ZipRecruiter'].map((site) => (
              <div key={site} className="flex items-center gap-2 px-3 py-2.5 rounded-lg bg-stone-50 dark:bg-stone-800 text-sm font-medium text-stone-700 dark:text-stone-300">
        ...
        <div className="bg-white dark:bg-stone-900 rounded-lg border border-stone-200/60 dark:border-stone-800 shadow-sm p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {FEATURES.map((f) => (
              <div key={f.key} className="flex items-start gap-3 px-3 py-3 rounded-lg bg-stone-50 dark:bg-stone-800">
                <span className="text-lg flex-shrink-0">{f.icon}</span>
                <div>
                  <p className="text-sm font-semibold text-stone-800 dark:text-stone-200">{t(`extension.featureList.${f.key}.title`)}</p>
                  <p className="text-xs text-stone-500 dark:text-stone-400 mt-0.5">{t(`extension.featureList.${f.key}.desc`)}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-4">
            {[1, 2, 3].map((step) => (
              <div key={step} className="flex items-start gap-3">
                <div className="w-7 h-7 rounded-full bg-stone-100 dark:bg-stone-800 text-stone-900 dark:text-stone-100 flex items-center justify-center text-xs font-bold flex-shrink-0">
                  {step}
                </div>
                <div>
                  <p className="text-sm font-semibold text-stone-800 dark:text-stone-200">{t(`extension.steps.${step}.title`)}</p>
                  <p className="text-xs text-stone-500 dark:text-stone-400 mt-0.5">{t(`extension.steps.${step}.desc`)}</p>
                </div>
              </div>
            ))}
          </div>
