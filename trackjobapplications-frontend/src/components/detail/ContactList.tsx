...
export default function ContactList({ applicationId }: Props) {
  ...
  return (
    <div className="space-y-3">
      ...
      {showAdd && (
        <div className="grid grid-cols-2 gap-2 p-3 bg-stone-50 dark:bg-stone-800 rounded-lg">
          <input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder={t('detail.contactList.namePlaceholder')} className="px-2 py-1.5 text-sm rounded border border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-900 dark:text-stone-100" />
      ...
      {loading ? (
        ...
        <p className="text-xs text-stone-400">{t('detail.contactList.empty')}</p>
      ) : (
        <div className="space-y-2">
          {contacts.map(c => (
            <div key={c.id} className="flex items-start justify-between p-2 bg-stone-50 dark:bg-stone-800 rounded-lg">
              <div>
