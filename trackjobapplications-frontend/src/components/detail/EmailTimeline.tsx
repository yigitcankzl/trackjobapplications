...
const TYPE_STYLES: Record<string, { bg: string; text: string }> = {
  ...
  general: { bg: 'bg-gray-50 dark:bg-gray-800', text: 'text-gray-700 dark:text-gray-400' },
}
  ...
  return (
    <div className="space-y-3">
      <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-100">
        ...
        {emails.length > 0 && <span className="ml-1.5 text-xs text-gray-400">({emails.length})</span>}
      </h3>

      {loading ? (
        <LoadingSpinner size="sm" centered />
      ) : emails.length === 0 ? (
        <p className="text-xs text-gray-400 dark:text-gray-500 italic">{t('detail.emails.empty')}</p>
      ) : (
